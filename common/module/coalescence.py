...
class Coalescence(nn.Module):
    ...
    def forward(self, input: Tensor, lengths: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        for i in range(0, max_seq_len, self._stride):
            ...
            if j - i < self._window_size:
                window = torch.zeros(batch_size, self._window_size, input_dim).to(input.device)
        ...
        if lengths is not None:
            lengths = (lengths.float() / self._stride).ceil().long()
